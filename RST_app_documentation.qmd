...
![Overview of data inputs, user inputs, functions, and application outputs for RST_app.](www/methods_figures/data_flow_v1_1.png){#fig-data_flow fig-alt="Flow chart showing data inputs, functions, and outputs of the RST_app."}
...
![Default view of the RST_app before data uploading.](www/methods_figures/user_inputs 1_2.png){#fig-user_inputs}

Users should set the following when using the application:

- **Upload data files:** Use this field to browse to the necessary RST data files (see Data formatting above) and load the files. You must select all the files at once to upload. When the necessary data sets are properly loaded, and if they are properly named, they will be displayed in the data list box (@fig-data_list).
...
![](www/methods_figures/data_list.png){#fig-data_list}
...
![Output of model comparisons showing models tested and ranked for two traps used in the example data set.](www/methods_figures/model_comparison.png){fig-alt="Figure showing output of model comparisons table."}

For each trap in the uploaded dataset the "Model Reports" tab will show the the efficiency models ran and display AIC and AICc values for comparison. The RST_app will automatically select the best fit model (based on AICc), but allows the user to select which efficiency trial model to utilize for each trap when estimating passage. A model marked as "temporal_df" notes a temporal spline followed by the number of degrees of freedom utilized. A model marked with "discharge" indicates discharge was used as a covariate of trap efficiency. So a model marked as "discharge_temporal_df4" includes discharge and a temporal spline with four degrees of freedom (a cubic spline with one internal knot).
...
![](www/methods_figures/passage_plot.png){#fig-passage_plot}
...
![](www/methods_figures/passage_table.png){#fig-passage_table}
...
![](www/methods_figures/catch_plot.png){#fig-catch_plot}

- **Efficiency plot:** displays plot of efficiency estimates for each rotary screw trap for period of survey. Displays estimates of efficiency derived from actual tests (in red) and imputed efficiency (in blue) and discharge on the alternate axis.

![](www/methods_figures/efficiency_plot.png){#fig-efficiency_plot}
